...
class FetchPopularProductsUseCase(private val repository: Repository): UseCase<Either<Failure<Nothing>, Products>, ProductsReq> {
    override suspend fun execute(input: ProductsReq): Either<Failure<Nothing>, Products> {
      return  repository.fetchPopularProducts(input)
    }
}