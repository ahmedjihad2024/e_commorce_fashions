...
class UpdateNameAndEmailUseCase(private val repository: Repository): UseCase<Either<Failure<Nothing>, Unit>, UserDetailsReq> {
    override suspend fun execute(input: UserDetailsReq): Either<Failure<Nothing>, Unit> {
        return repository.updateNameAndEmail(input)
    }
}